#!/usr/bin/env python3
"""100% Complete Rebuild of 1980 Lake Placid Winter Olympics Channel"""
...
from streamtv.importers.channel_importer import import_channels_from_yaml
import logging
...
async def fetch_metadata_for_url(stream_manager: StreamManager, url: str) -> dict:
    ...
    try:
        ...
        # Find the specific file in video_files
        video_files = item_info.get('video_files', [])
        target_file = None
        if decoded_filename:
            for vf in video_files:
                if vf['name'] == decoded_filename:
                    target_file = vf
                    break
        ...
        duration = None
        if target_file:
            # Archive.org sometimes has duration in metadata
            pass
...
async def complete_rebuild(db: Session):
    """100% Complete Rebuild"""
    logger.info("=" * 80)
    logger.info("100% COMPLETE REBUILD: 1980 Lake Placid Winter Olympics Channel")
    logger.info("=" * 80)
    
    stream_manager = StreamManager()
    
    # STEP 1: Reset channel to ON_DEMAND and reset creation time
    logger.info("\n[STEP 1] Resetting channel configuration...")
    channel = db.query(Channel).filter(Channel.number == '1980').first()
    if not channel:
        logger.error("Channel 1980 not found!")
        return
    
    # Set to ON_DEMAND mode
    channel.playout_mode = PlayoutMode.ON_DEMAND
    # Reset creation time to NOW (so timeline starts fresh)
    now = datetime.now(timezone.utc)
    channel.created_at = now
    channel.updated_at = now
    db.commit()
    logger.info(f"  ✓ Channel set to ON_DEMAND mode")
    logger.info(f"  ✓ Channel creation time reset to: {now}")
    
    # STEP 2: Delete ALL existing 1980 Olympics data
    logger.info("\n[STEP 2] Deleting ALL existing 1980 Olympics data...")
    
    # Delete all collection items for 1980 collections
    ...
    for collection in collections:
        ...
        logger.info(f"  ✓ Deleted collection: {collection.name} ({items_count} items)")
    ...
    db.commit()
    logger.info(f"  ✓ Deleted {deleted_collections} collections, {deleted_items} collection items, {deleted_media} media items")
    ...
    for day in sorted(urls_by_day.keys()):
        logger.info(f"    Day {day:02d}: {len(urls_by_day[day])} files")
    ...
    # STEP 5: Re-import YAML schedule
    logger.info("\n[STEP 5] Re-importing YAML schedule...")
    schedule_file = Path(__file__).parent.parent / "schedules" / "mn-olympics-1980.yml"
    if schedule_file.exists():
        try:
            import_channels_from_yaml(schedule_file, db)
            logger.info("  ✓ YAML schedule re-imported successfully")
        except Exception as e:
            logger.error(f"  ✗ Failed to re-import YAML: {e}")
            import traceback
            traceback.print_exc()
    else:
    ...
    logger.info("\n" + "=" * 80)
    logger.info("✅ 100% REBUILD COMPLETE!")
    logger.info("=" * 80)
    logger.info(f"  ✓ Channel set to ON_DEMAND mode")
    logger.info(f"  ✓ Channel creation time reset")
    logger.info(f"  ✓ Deleted all old collections and media")
    logger.info(f"  ✓ Imported {imported_count} new media items")
    logger.info(f"  ✓ Created {len(urls_by_day)} collections")
    logger.info(f"  ✓ YAML schedule re-imported")
    logger.info("\n📋 NEXT STEPS:")
    ...
    logger.info("  3. Test the channel to verify it starts correctly")
    logger.info("=" * 80)
