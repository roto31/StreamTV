...
- Plex spinner after StreamTV restart until manual retune

### Solutions

1. **After StreamTV restart:** Plex does not auto-retune. Stop and reselect the
   channel in the Plex guide. See
   [Plex Live TV recovery](../plex/PLEX_LIVE_TV_RECOVERY.md).

2. **When every channel fails to tune:** restart Plex Tuner Service or Plex
   Media Server (see recovery doc above).
