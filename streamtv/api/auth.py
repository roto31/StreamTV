...
async def archive_org_login_page_legacy(request: Request):
    ...
    return templates.TemplateResponse(request, "auth_archive.html", {
...
async def archive_org_login_page(request: Request):
    ...
    preferred = config.archive_org.username
    keychain_creds = None
    if preferred:
        keychain_creds = get_credentials_from_keychain("archive.org", account=preferred)
    ...
    if keychain_creds:
        username = resolve_archive_org_login_email(keychain_creds[0], preferred)
    else:
        username = config.archive_org.username or ""
    
    return templates.TemplateResponse(request, "auth_archive_org.html", {
...
async def youtube_login_page(request: Request):
    """YouTube login page"""
    return templates.TemplateResponse(request, "auth_youtube.html", {
...
async def pbs_login_page(request: Request):
    ...
    return templates.TemplateResponse(request, "auth_pbs.html", {
...
async def youtube_oauth_start(request: Request, skip_passkey: bool = False):
    ...
    if not skip_passkey and passkey_manager:
        ...
        return templates.TemplateResponse(request, "auth_youtube_passkey.html", {
"oauth_ready": False,
    ...
    try:
        ...
        if not config.youtube.oauth_client_id:
            return templates.TemplateResponse(request, "auth_youtube_oauth_setup.html", {
"needs_setup": True
})
...
async def youtube_oauth_callback(request: Request, code: Optional[str] = None, state: Optional[str] = None, error: Optional[str] = None):
    ...
    if error:
        return templates.TemplateResponse(request, "auth_youtube_oauth_error.html", {
"error": error
})
    ...
    try:
        ...
        return templates.TemplateResponse(request, "auth_youtube_oauth_success.html", {
