...
from ..hdhomerun.api import guide_name_for_channel
from ..scheduling import ScheduleParser, ScheduleEngine
from ..scheduling.engine import playout_shuffle_seed
from ..scheduling.playout_timeline import (
    ...
    resolve_stream_playout_position,
...
def _format_xmltv_time(dt: datetime) -> str:
    """Format datetimes as local wall-clock XMLTV stamps for Plex HDHomeRun clients."""
...
def _is_ytimg_thumbnail_url(thumb_raw: str) -> bool:
    """Return True when thumbnail URL host is YouTube image CDN (parsed, not substring)."""
    if not thumb_raw:
        return False
    candidate = thumb_raw
    ...
    return host == "ytimg.com" or host.endswith(".ytimg.com")


def _epg_live_air_anchor(
    channel_manager: Optional[object],
    playback_pos: Optional[object],
    channel_number: str,
    guide_index: int,
...
def _build_channel_playlist_content(
    ...
    *,
    ...
    """Build IPTV channel-list playlist; optional UHF-compatible M3U8 header."""
    m3u_content = "#EXTM3U\n"
    if uhf_m3u8:
        m3u_content += "#EXT-X-VERSION:3\n"

    use_ts = mode not in ("hls", "mixed")
...
async def get_channel_playlist(
    ...
    try:
        ...
        _normalize_channel_playout_modes(channels)
        ...
        base_url = resolve_hdhomerun_base_url(request)
        m3u_content = _build_channel_playlist_content(
            channels, base_url, mode, access_token
        )
        ...
        error_m3u = "#EXTM3U\n"
        error_m3u += "#EXTINF:-1,Error: Unable to generate playlist\n"
        error_m3u += "#\n"
        return Response(
            content=error_m3u,
...
def _normalize_channel_playout_modes(channels: List[Channel]) -> None:
    """Ensure playout_mode is a PlayoutMode enum on each channel."""
    from ..database.models import PlayoutMode

    for channel in channels:
        if not isinstance(channel.playout_mode, str):
            continue
        try:
            normalized = channel.playout_mode.lower().replace('-', '_')
            matched = False
            for playout_mode in PlayoutMode:
                if playout_mode.value.lower() == normalized:
                    channel.playout_mode = playout_mode
                    matched = True
                    break
            if not matched:
            ...
            logger.warning(
                f"Invalid playout_mode '{channel.playout_mode}' for channel "
                f"{channel.number}, defaulting to CONTINUOUS: {e}"
            )
            channel.playout_mode = PlayoutMode.CONTINUOUS


@router.get("/iptv/channels.m3u8")
async def get_channel_playlist_uhf(
    access_token: Optional[str] = None,
    request: Request = None,
    db: Session = Depends(get_db),
):
    """IPTV playlist for strict M3U8 clients (e.g. UHF): EXT-X-VERSION + MPEG-TS URLs."""
    try:
        if config.security.api_key_required:
            if config.security.access_token is None:
                logger.debug(
                    "API key required but access_token not set - allowing access for M3U8"
                )
            elif access_token != config.security.access_token:
                raise HTTPException(status_code=401, detail="Invalid access token")
        ...
        base_url = resolve_hdhomerun_base_url(request)
        m3u_content = _build_channel_playlist_content(
            channels, base_url, "ts", access_token, uhf_m3u8=False
        )
        return Response(
            content=m3u_content,
            media_type="application/vnd.apple.mpegurl",
        ...
        logger.error(f"Error generating UHF M3U8 playlist: {e}", exc_info=True)
        error_m3u = "#EXTM3U\n#EXT-X-VERSION:3\n"
        error_m3u += "#EXTINF:-1,Error: Unable to generate playlist\n#\n"
        return Response(
            content=error_m3u,
...
async def get_epg(
    ...
    try:
        ...
        for channel in channels:
            ...
            guide_name = guide_name_for_channel(channel)
            xml_content += f'  <channel id="{_xml(channel_id)}">\n'
            
            # Tunarr-style display-name order (number+name, number, name) for Plex HDHomeRun mapping
            combined_name = f"{channel_id} {guide_name}".strip()
            xml_content += f'    <display-name>{_xml(combined_name)}</display-name>\n'
            xml_content += f'    <display-name>{_xml(channel_id)}</display-name>\n'
            xml_content += f'    <display-name>{_xml(guide_name)}</display-name>\n'
        ...
        for channel in channels:
            ...
            # Prefer live in-memory anchor from ChannelManager so EPG matches skips/timeline jumps.
            playout_start_time = None
            channel_manager = None
            if request is not None:
                channel_manager = getattr(request.app.state, "channel_manager", None)
            if channel_manager:
                live_anchor = channel_manager.get_live_playout_start_time(str(channel.number))
                if live_anchor:
                    playout_start_time = live_anchor
                    logger.debug(
                        f"Channel {channel.number}: Using live playout_start_time "
                        f"{playout_start_time} for EPG"
                    )
            if playout_start_time is None:
                ...
                try:
                    ...
                    schedule_engine = ScheduleEngine(
                        ...
                        seed=playout_shuffle_seed(
                            str(channel.number), playout_start_time, 0
                        ),
                    )
                    ...
                    logger.info(
                        f"Generated {len(schedule_items)} stream / {len(timeline_items)} timeline "
                        f"schedule items for channel {channel.number}"
                    )

                    stamp_cached_durations(schedule_items)
                    guide_position = resolve_stream_playout_position(
                        schedule_items, timeline_items, playout_start_time, now
                    )
                    guide_index = int(guide_position["item_index"])
                    live_item_index, live_item_start_time = _epg_live_air_anchor(
                        channel_manager,
                        playback_pos,
                        str(channel.number),
                        guide_index,
                    ...
                    logger.info(
                        f"Channel {channel.number}: EPG aligned to playout item "
                        f"{position['item_index']}/{len(schedule_items)} "
                        f"(guide {guide_idx}/{len(schedule_items)}, "
                        f"timeline {timeline_idx}/{len(timeline_items)}, "
                        f"cycle {position['cycle_position']/3600:.1f}h)"
                    )
            ...
            if not schedule_items:
                ...
                if not schedule_items and playlists:
                    ...
                    if items:
                        ...
                        if playlist_schedule:
                            pst = (
                                playback_pos.playout_start_time
                                if playback_pos and playback_pos.playout_start_time
                                else now
                            )
                            schedule_items = assign_schedule_times_from_playout(
                                playlist_schedule, pst, now
                            )
                            pos = compute_continuous_playout_position(playlist_schedule, pst, now)
                            logger.info(
                                f"Channel {channel.number}: EPG playlist aligned to item "
                                f"{pos['item_index']}/{len(playlist_schedule)}"
                            )
            ...
            if not schedule_items:
                ...
                start_str = _format_xmltv_time(now)
                end_str = _format_xmltv_time(end_time)
                channel_id = str(channel.number).strip()
            ...
            for schedule_item in schedule_items:
                ...
                # Clean up title to remove collection suffixes (do not clobber meta show_name)
                display_title = title
                title_clean = re.sub(r'\s+[Ss]\d+[Ee]\d+$', '', display_title)
                if title_clean != display_title:
                ...
                if is_currently_playing or is_future:
                    programme_count += 1
                    start_str = _format_xmltv_time(start_time)
                    end_str = _format_xmltv_time(end_time_prog)
        ...
        error_xml += '  <error>Unable to generate EPG</error>\n'
        error_xml += '</tv>\n'
        return Response(
            content=error_xml,
            media_type="application/xml; charset=utf-8",
...
async def get_hls_stream(
    ...
    if schedule_file:
        try:
            ...
            schedule_engine = ScheduleEngine(
                db,
                seed=playout_shuffle_seed(str(channel.number), None, 0),
            )
            schedule_items = schedule_engine.generate_playlist_from_schedule(
                channel, parsed_schedule, max_items=None
            )
        ...
        if channel_manager:
            ...
            async def generate():
                try:
                    ...
                    logger.info(
                        f"IPTV TS stream cancelled for channel {channel_number} "
            ...
            async def generate():
                try:
                    ...
                    logger.info(
                        f"IPTV on-demand stream cancelled for channel {channel_number} "
                        f"(client disconnected)"
                    )
                    return
                except Exception as e:
