...
from ..api.schemas import MediaItemCreate, MediaItemFromUrl, MediaItemResponse, MediaItemUpdate
...
async def create_media(body: MediaItemFromUrl, db: Session = Depends(get_db)):
    """Add a new media item from URL"""
    media_url = body.url
    # Check if URL already exists
    existing = db.query(MediaItem).filter(MediaItem.url == media_url).first()
    if existing:
        return existing
    
    # Detect source and get media info
    source = stream_manager.detect_source(media_url)
    if source.value == "unknown":
        raise HTTPException(status_code=400, detail="Unsupported media source")
    
    try:
        # Get media information
        media_info = await stream_manager.get_media_info(media_url, source)
        
        # Extract source ID
        source_id = media_url
        if source.value == "youtube" and stream_manager.youtube_adapter:
            source_id = stream_manager.youtube_adapter.extract_video_id(media_url) or media_url
        elif source.value == "archive_org" and stream_manager.archive_org_adapter:
            source_id = stream_manager.archive_org_adapter.extract_identifier(media_url) or media_url
...
def update_media(
    ...
    """Update editable fields on a media item."""
    media_item = db.query(MediaItem).filter(MediaItem.id == media_id).first()
    if not media_item:
        raise HTTPException(status_code=404, detail="Media item not found")
