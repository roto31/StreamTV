...
from ..api.schemas import PlaylistCreate, PlaylistResponse, PlaylistItemResponse
...
def get_playlist_items(playlist_id: int, db: Session = Depends(get_db)):
    """Get ordered items for a playlist (with nested media_item)."""
    playlist = db.query(Playlist).filter(Playlist.id == playlist_id).first()
    if not playlist:
        raise HTTPException(status_code=404, detail="Playlist not found")
    return (
