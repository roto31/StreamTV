...
async def import_channels_yaml(
    ...
    try:
        ...
        # Import channels (reuse request session so returned objects stay attached)
        channels = await import_channels_from_yaml(tmp_path, validate=validate, db_session=db)
...
async def import_channels_yaml_path(
    ...
    if not config.security.allow_local_path_import:
        raise HTTPException(
            status_code=403,
            detail="Local path import is disabled. Set security.allow_local_path_import: true in config.yaml.",
        )
    ...
    try:
        ...
        # Import channels (reuse request session so returned objects stay attached)
        channels = await import_channels_from_yaml(yaml_path, validate=validate, db_session=db)
