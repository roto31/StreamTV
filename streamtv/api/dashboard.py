...
async def check_pbs_status() -> Dict:
    """Check PBS authentication and connection status."""
    status = {
        "authenticated": False,
        "connection_ok": False,
        "status": "red",
    ...
    try:
        if config.pbs.cookies_file:
            cookies_path = Path(config.pbs.cookies_file)
            if cookies_path.exists():
                status["authenticated"] = True
        elif config.pbs.use_authentication and config.pbs.username:
            status["authenticated"] = True

        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                response = await client.get("https://www.pbs.org", follow_redirects=True)
                status["connection_ok"] = response.status_code == 200
        except Exception as e:
            logger.warning(f"PBS connection check failed: {e}")
            status["connection_ok"] = False

        status["errors"] = parse_log_errors("pbs")
        ...
        logger.error(f"Error checking PBS status: {e}")
        status["errors"] = [str(e)]
        status["status"] = "red"
...
async def get_dashboard_status():
    ...
    try:
        ...
        archive_org_status, youtube_status, pbs_status, streamtv_api_status, auth_apis_status = await asyncio.gather(
        ...
        if isinstance(pbs_status, Exception):
            pbs_status = {"status": "red", "errors": [str(pbs_status)], "authenticated": False, "connection_ok": False}
        ...
        return {
            ...
            "pbs": {"status": "red", "errors": [f"Status check failed: {str(e)}"], "authenticated": False, "connection_ok": False},
