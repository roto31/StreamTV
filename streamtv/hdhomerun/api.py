...
from .urls import resolve_hdhomerun_base_url
...
# Use a SiliconDust-style model ID so Plex accepts the virtual tuner.
HDHOMERUN_MODEL = "HDHR4-2US"
HDHOMERUN_FIRMWARE = "20190621"

_GENERIC_GUIDE_NAMES = frozenset(
    {"movies", "music", "news", "sports", "comedy", "classics", "reality", "explore"}
)


def guide_name_for_channel(channel: Channel) -> str:
    """HDHomeRun GuideName / primary XMLTV display-name (must match lineup.json)."""
    ...
    if guide_name and guide_number:
        ...
        if name_stripped.startswith(number_str):
            ...
            if candidate and candidate.lower() not in _GENERIC_GUIDE_NAMES:
                guide_name = candidate
            elif candidate:
                guide_name = f"{number_str}s {candidate}"
    return guide_name
...
async def discover(request: Request, db: Session = Depends(get_db)):
    ...
    epg_url = f"{base_url}/xmltv.xml"
    if config.tuner_manager.enabled and config.tuner_manager.merged_guide:
        epg_url = f"{base_url}/tuners/guide.xml"
    
    response = {
        "FriendlyName": config.hdhomerun.friendly_name,
        "ModelNumber": HDHOMERUN_MODEL,
        "FirmwareName": f"hdhomerun4_atsc",
...
async def lineup(request: Request, db: Session = Depends(get_db)):
    ...
    # Channel enum-like fields are stored as String columns with @reconstructor
    # normalization — no raw-SQL workaround needed.
    channels = (
        db.query(Channel)
    ...
    for channel in channels:
        ...
        stream_url = f"{base_url}/auto/v{channel.number}"
...
async def stream_channel(
    ...
    try:
        ...
        if channel_manager:
            ...
            logger.warning(
                "HDHomeRun: ChannelManager not available for channel %s",
                channel_number,
            )
            raise HTTPException(
                status_code=503,
                detail={
                    "error": "channel_manager_unavailable",
                    "message": "Continuous playout is still starting. Retry tune shortly.",
