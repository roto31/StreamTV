...
class PlexAPIClient:
    ...
    def __init__(self, base_url: str, token: Optional[str] = None):
        ...
        Args:
            base_url: Plex Media Server base URL (e.g., "http://192.0.2.1:32400")
    ...
    async def reload_dvr_guide(self, dvr_id: str) -> bool:
        """Force Plex to re-fetch XMLTV from configured HDHomeRun/EPG URL."""
        try:
            url = f"{self.base_url}/livetv/dvrs/{dvr_id}/reloadGuide"
            response = await self._client.post(url, headers=self._get_headers())
            if response.status_code == 200:
                logger.info("Plex reloadGuide succeeded for DVR %s", dvr_id)
                return True
            logger.warning(
                "Plex reloadGuide failed for DVR %s: HTTP %s",
                dvr_id,
                response.status_code,
            )
            return False
        except Exception as e:
            logger.warning("Plex reloadGuide error for DVR %s: %s", dvr_id, e)
            return False
