"""Schedule engine for StreamTV Schedule v1 playout sequences."""
...
from .media_format import filter_media_items

logger = logging.getLogger(__name__)


def playout_shuffle_seed(
    channel_number: str,
    playout_start_time: Optional[datetime] = None,
    cycle_index: int = 0,
) -> int:
    """Deterministic seed shared by EPG generation and continuous playout.

    Uses hashlib (not ``hash()``) so the seed is stable across processes —
    Python's ``hash()`` is randomized per process by default.
    """
    import hashlib

    anchor = (
        playout_start_time.isoformat(timespec="seconds")
        if playout_start_time
        else "no-anchor"
    )
    raw = f"{channel_number}|{anchor}|{cycle_index}"
    digest = hashlib.sha256(raw.encode("utf-8")).hexdigest()
    return int(digest[:16], 16) % (2**31 - 1)


class ScheduleEngine:
    """Engine for executing StreamTV Schedule v1 sequences."""
    ...
    def __init__(self, db: Session, seed: Optional[int] = None):
        ...
        self._seed = seed if seed is not None else random.randint(1, 1000000)
    ...
    def get_sequence_media(self, sequence_key: str, schedule: ParsedSchedule) -> List[MediaItem]:
        ...
        for item in sequence_items:
            ...
            if content_key and content_key in schedule.content_map:
                ...
                items = filter_media_items(items, schedule, content_key)
    ...
    def resolve_sequence_item(
        ...
        if 'sequence' in item:
            ...
            for media_item in media_items:
                ...
                item_time += timedelta(seconds=media_item.duration or 1800)
        ...
        if 'all' in item:
            ...
            if content_key in schedule.content_map:
                ...
                media_items = filter_media_items(media_items, schedule, content_key)
    ...
    def generate_playlist_from_schedule(
        ...
        # Continuous playout loops via playout_timeline modulo — one schedule cycle only.
        # repeat: true means "loop the sequence", not "expand to max_items rows".
        if max_items and len(base_playlist_items) > max_items:
            playlist_items = base_playlist_items[:max_items]
        else:
            playlist_items = base_playlist_items

        logger.info(
            f"Generated {len(playlist_items)} items from schedule: {schedule.name} "
            f"(repeat={repeat}, cycle_only=True)"
        )
