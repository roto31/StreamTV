...
class ParsedSchedule:
    """Parsed StreamTV Schedule v1 data structure (playout layer)."""
    ...
    def __init__(self, name: str, description: str):
        ...
        self.imports: List[str] = []  # Import other YAML schedule fragments
        self.reset: List[Dict[str, Any]] = []  # Reset instructions
        self.media_defaults: Dict[str, Any] = {}  # preferred_extension, mp4_only, etc.
...
class ScheduleParser:
    ...
    def parse_file(file_path: Path, base_dir: Optional[Path] = None) -> ParsedSchedule:
        ...
        media_block = data.get('media')
        if isinstance(media_block, dict):
            schedule.media_defaults = {
                k: media_block[k]
                for k in ('preferred_extension', 'mp4_only', 'preferred_format')
                if k in media_block
            }
        ...
        for import_path in schedule.imports:
            try:
                ...
                if import_file.exists():
                    ...
                    if imported_schedule.media_defaults and not schedule.media_defaults:
                        schedule.media_defaults = dict(imported_schedule.media_defaults)
        ...
        for content_def in content_list:
            ...
            if key:
                ...
                for opt in ('preferred_extension', 'mp4_only', 'preferred_format'):
                    if opt in content_def:
                        entry[opt] = content_def[opt]
                schedule.content_map[key] = entry
    ...
    def find_schedule_file(channel_number: str) -> Optional[Path]:
        ...
        possible_names = [
            ...
            # Named schedule aliases (inventory channel number still keys lookup)
            "magnum-pi-schedule.yml" if channel_number == "80" else "",
        ]
