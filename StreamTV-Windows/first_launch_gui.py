#!/usr/bin/env python3
"""
First Launch GUI for StreamTV Windows
...
class FirstLaunchGUI:
    ...
    def check_dependencies(self):
        ...
        if python_path:
            ...
            if (self.bundled_dir / "python" / "python.exe").exists():
                self.extract_python()
            else:
                self.status_label.config(
                    text="Python not found. Please install Python 3.10+ from python.org",
                    fg="red"
                )
        ...
        if ffmpeg_path:
            ...
            if (self.bundled_dir / "ffmpeg" / "ffmpeg.exe").exists():
                self.extract_ffmpeg()
            else:
                self.status_label.config(
    ...
    def check_python(self):
        ...
        bundled_python = self.app_support / "python" / "python.exe"
    ...
    def check_ffmpeg(self):
        ...
        bundled_ffmpeg = self.app_support / "ffmpeg" / "ffmpeg.exe"
    ...
    def extract_python(self):
        ...
        try:
            ...
            # Store path
            python_exe = extracted_python_dir / "python.exe"
            if python_exe.exists():
                with open(self.app_support / "python_path.json", "w") as f:
                    json.dump({"path": str(python_exe)}, f)
                
                self.python_installed = True
                self.python_status.config(text="✅")
                self.python_label.config(text=f"Python (Bundled: {python_exe})")
    ...
    def extract_ffmpeg(self):
        ...
        try:
            ...
            # Copy ffmpeg.exe
            ffmpeg_exe = bundled_ffmpeg_dir / "ffmpeg.exe"
            if ffmpeg_exe.exists():
                shutil.copy2(ffmpeg_exe, extracted_ffmpeg_dir / "ffmpeg.exe")
            
            # Copy ffprobe.exe
            ffprobe_exe = bundled_ffmpeg_dir / "ffprobe.exe"
            if ffprobe_exe.exists():
                shutil.copy2(ffprobe_exe, extracted_ffmpeg_dir / "ffprobe.exe")
            
            # Store path
            extracted_ffmpeg = extracted_ffmpeg_dir / "ffmpeg.exe"
